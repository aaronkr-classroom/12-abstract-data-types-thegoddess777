...
using namespace std;
// std::max, uninitialized_fill, uninitialized_copy ...
...
private:
	...
	iterator avail; // 마지막으로 만든 요소 다음을 가리키는 포인터
	...
	template <class T> void Vec<T>::create(const_iterator i, const_iterator j) {
		...
		limit = uninitialized_copy(i, j, data); // allocator 비멤버 함수
	}
	...
	template <class T> void Vec<T>::grow() {
		// 저장 공간을 늘릴 때에는 현재 사용하는 저장 공간의 2배를 할당
